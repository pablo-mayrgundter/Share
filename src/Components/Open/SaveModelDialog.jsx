...
export default function SaveModelDialog({isDialogDisplayed, setIsDialogDisplayed, navigate, orgNamesArr}) {
    ...
    const selectOrg = async (org) => {
      ...
      let repos
      if (org === user.nickname) {
        repos = await getUserRepositories(accessToken, org)
      } else {
        repos = await getRepositories(org, accessToken)
      }
    ...
    const selectRepo = async (repo) => {
      ...
      const {files, directories} = await getFilesAndFolders(repoNamesArr[repo], selectedOrgName, '/', accessToken)
    ...
    const selectFolder = async (folderIndex) => {
      ...
      const {files, directories} = await getFilesAndFolders(repoName, selectedOrgName, newPath, accessToken)
      const fileNames = files.map((f) => f.name)
      const directoryNames = directories.map((directory) => directory.name)
...
function fileSave(
  ...
  if (file instanceof File) {
    ...
    if (commitHash !== null) {
      // save to opfs
      if (opfsIsAvailable) {
        const opfsResult = writeSavedGithubModelOPFS(file, pathWithFileName, commitHash, orgName, repoName, branchName)
